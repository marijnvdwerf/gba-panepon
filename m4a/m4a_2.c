...
extern char SoundMainBuf[0x400];

extern struct SoundInfo m4a_sound;
struct PokemonCrySong gPokemonCrySongs[MAX_POKEMON_CRIES];
struct MusicPlayerInfo gPokemonCryMusicPlayers[MAX_POKEMON_CRIES];
void *ExtMPlyJmpTbl[36];
struct CgbChannel m4a_cgbchn[4];
...
u32 MidiKey2fr(struct WaveData *wav, u8 key, u8 fineAdjust)
{
    ...
    val1 = ScaleTable[key];
    val1 = FreqTable[val1 & 0xF] >> (val1 >> 4);

    val2 = ScaleTable[key + 1];
    val2 = FreqTable[val2 & 0xF] >> (val2 >> 4);
...
void m4aSoundInit(void)
{
    ...
    CpuCopy32((void *)((s32)SoundMainRAM & ~1), SoundMainBuf, sizeof(SoundMainBuf));

    SoundInit(&m4a_sound);
    MPlayExtender(m4a_cgbchn);
    m4aSoundMode(0x94C700);
    ...
    for (i = 0; i < NUM_MUSIC_PLAYERS; i++)
    {
        ...
        mplayInfo->memAccArea = m4a_memacc_area;
    }
}
...
void MPlayExtender(struct CgbChannel *cgbChans)
{
    ...
    ExtMPlyJmpTbl[8] = ply_memacc;
    ExtMPlyJmpTbl[17] = ply_lfos_rev01;
    ExtMPlyJmpTbl[19] = ply_mod_rev01;
    ExtMPlyJmpTbl[28] = ply_xcmd;
    ExtMPlyJmpTbl[29] = ply_endtie_rev01;
    ExtMPlyJmpTbl[30] = SampFreqSet_rev01;
    ExtMPlyJmpTbl[31] = TrackStop_rev01;
    ExtMPlyJmpTbl[32] = FadeOutBody_rev01;
    ExtMPlyJmpTbl[33] = TrkVolPitSet_rev01;

    soundInfo->cgbChans = (struct CgbChannel *)cgbChans;
    soundInfo->CgbSound = CgbSound;
    soundInfo->CgbOscOff = CgbOscOff;
    soundInfo->MidiKeyToCgbFreq = MidiKey2CgbFr;
    soundInfo->maxLines = MAX_LINE;
...
{
    void (*func)(void *) = *(&ExtMPlyJmpTbl[34]);
    func(x);
}

void Clear64byte(void *x)
{
    void (*func)(void *) = *(&ExtMPlyJmpTbl[35]);
    func(x);
}
...
void SoundInit(struct SoundInfo *soundInfo)
{
    ...
    soundInfo->plynote = (u32)ply_note_rev01;
    soundInfo->CgbSound = DummyFunc_rev;
    soundInfo->CgbOscOff = (void (*)(u8))DummyFunc_rev;
    soundInfo->MidiKeyToCgbFreq = (u32 (*)(u8, u8, u8))DummyFunc_rev;
    soundInfo->ExtVolPit = (u32)DummyFunc_rev;

    MPlayJumpTableCopy(ExtMPlyJmpTbl);

    soundInfo->MPlayJumpTable = (u32)ExtMPlyJmpTbl;

    SampFreqSet_rev01(SOUND_MODE_FREQ_13379);

    soundInfo->ident = ID_NUMBER;
}

void SampFreqSet_rev01(u32 freq)
{
    struct SoundInfo *soundInfo = SOUND_INFO_PTR;

    freq = (freq & 0xF0000) >> 16;
    soundInfo->freq = freq;
    soundInfo->pcmSamplesPerVBlank = pcmVBtbl_rev[freq - 1];
...
void MPlayStart(struct MusicPlayerInfo *mplayInfo, struct SongHeader *songHeader)
{
        ...
        || (mplayInfo->priority <= songHeader->priority))
    {
        ...
        {
            TrackStop_rev01(mplayInfo, track);
...
u32 MidiKey2CgbFr(u8 chanNum, u8 key, u8 fineAdjust)
{
    if (chanNum == 4)
    {
        if (key <= 20)
    ...
    else
    {
        ...
        val1 = CgbScTable[key];
        val1 = CgbFrTable[val1 & 0xF] >> (val1 >> 4);

        val2 = CgbScTable[key + 1];
        val2 = CgbFrTable[val2 & 0xF] >> (val2 >> 4);
